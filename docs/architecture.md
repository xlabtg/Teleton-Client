...
- Secure Data Deletion is represented by `src/foundation/secure-data-deletion.mjs`. It builds local deletion plans for account, cache, agent, and wallet scopes, records platform storage locations and filesystem limitations, requires exact irreversible confirmation text before execution hooks run, supports cache recovery window behavior, and keeps platform limitation review release-blocking until a human security reviewer approves it.
...
- End-to-end workflow coverage is represented by `src/foundation/e2e-workflow-harness.mjs`. It composes the TDLib auth and messaging adapter, agent action proposal/confirmation contracts, and TON transfer draft plus confirmation workflow into one mock-mode scenario that local CI can run without production accounts. Live mode stays disabled until protected CI supplies secure TDLib, agent transport, and TON provider references, and failures carry redacted logs plus screenshots when a platform capture hook is available.
...
The shared secure deletion contract defines four local scopes: account, cache, agent, and wallet. Account deletion covers TDLib session state, message database snapshots, device-only authentication caches, and secure references for this installation. Cache deletion covers cached media, previews, temporary uploads, offline shell cache entries, and derived thumbnails. Agent deletion covers encrypted memory snapshots, vector indexes, runtime scratch state, and the local agent memory encryption key reference. Wallet deletion covers local wallet metadata, transaction caches, pending drafts, WalletConnect sessions, and wallet provider secure references. None of these flows deletes a Telegram account, remote messages, external wallet provider account, or public blockchain history.

`createSecureDataDeletionPlan()` records platform-specific storage locations for Android, iOS, desktop, and web wrappers. It exposes irreversible effects and requires the user to enter exact confirmation text such as `DELETE LOCAL ACCOUNT, CACHE, AGENT, AND WALLET DATA` before `executeSecureDataDeletionPlan()` will call adapter hooks. Secret-bearing scopes use immediate secure reference destruction followed by local file or database removal. Cache scope can use a recovery window: cache entries are hidden immediately, remain restorable until the deadline, and are purged when the window expires.

Secure deletion is a best-effort local workflow. Android and iOS flash storage, APFS snapshots, filesystem journals, desktop SSD wear-leveling, search indexes, browser IndexedDB and CacheStorage internals, and user or system backups can retain remnants outside app control. The plan keeps `humanReview.releaseBlocker` true until a human security reviewer confirms the wrapper storage inventory, backup behavior, diagnostics redaction, and documented filesystem limitations for the target platform.
...
This PR implements only the foundation layer, epic decomposition workflow, baseline TDLib adapter boundary, TDLib two-factor state controller, message database encryption contract, secure data deletion contract, hardware security key contract, Android wrapper contract, iOS wrapper contract, desktop wrapper contract, shared input action map, responsive tablet layout contract, Web PWA wrapper contract, settings synchronization foundation contract, offline synchronization foundation contract, local agent runtime lifecycle contract, agent IPC bridge contract, local agent memory encryption contract, automatic token refresh contract, mock-backed TON wallet adapter boundary, mock-backed TON swap adapter boundary, and mock-mode E2E workflow harness. Native Gradle, Xcode, or Electron sources, concrete web UI assets, live TDLib integration, concrete agent process packaging, concrete IPC transports, concrete secure storage bindings, hardware authenticator bridges, production sync providers, and live TON operations remain tracked by the generated subtasks in `config/epic-subtasks.json`.
